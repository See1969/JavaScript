...
let wordsList = function (str, subStr) {
  let arr = str.replace(/[.,!?;:"']/g, '').toLowerCase().split(' ').filter(el => el.includes(subStr));

  const unicWord = new Set(arr);
...
let getLocalDate = function (date, isSeconds = false, isISO = false) {
  const year = isISO ? date.getFullYear() : String(date.getFullYear());
 ...
 */
let getDateAgo = (dt, ds) => {
  const date = new Date(dt)

  date.setDate(date.getDate() - ds);
